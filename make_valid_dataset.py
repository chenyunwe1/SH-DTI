...
def mk_valid_dataset(valid_data_path,nf_data_name,mask_name,noisy_data_save_path,gt_save_path):
    idx = 0
    for file_name in os.listdir(valid_data_path):
        print(file_name)
        nf_data_path = valid_data_path + '/' + file_name + '/' + nf_data_name
        mask_path = valid_data_path + '/' + file_name + '/' + mask_name
...
def spherical_harmonics_representation(Y_inv_name,noisy_data_save_path,gt_save_path,sh_coe_save_path):
    Y_inv = np.load(Y_inv_name)
    ...
    for i in range(980):
        ...
        data_sph_noisy = tf.squeeze(tf.matmul(Y_inv,b0_nor_noisy))
        mask = np.expand_dims(mask,axis=3)  
        data_ = mask*np.array(data_sph_noisy)
        data_ = data_.astype(np.float32)
        save_path_sph = os.path.join(sh_coe_save_path, 'sph_%04d.npy' % i)
        np.save(save_path_sph,data_)
        print(data_.shape)
    flag = 1
    return flag
...
if __name__ == '__main__':
    ...
    # select patch and add noise
    a = mk_valid_dataset(valid_data_path,nf_data_name,mask_name,noisy_data_save_path,gt_save_path)
    print(a)
    # spherical harmonics representation
    b = spherical_harmonics_representation(Y_inv_name,noisy_data_save_path,gt_save_path,sh_coe_save_path)
    print(b)
